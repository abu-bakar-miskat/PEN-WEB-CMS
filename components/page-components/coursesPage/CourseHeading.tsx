...
import { useCourseHeadingSection } from "@/lib/hooks/pages/courses/useCourseSections";


const CourseHeading = () => {
  const { data: sectionHeading, isLoading } = useCourseHeadingSection();
  const fields = sectionHeading?.content;

  if (isLoading || !fields || !sectionHeading) {
    return (
