...
import { useAllCourseHeadingSection } from "@/lib/hooks/pages/courses/useCourseSections";

const AllCourseHeading = () => {
  const { data: sectionHeading, isLoading } = useAllCourseHeadingSection();
  ...
  const { tag, title, sub_title } = fields;
//   console.log(Object.keys(sectionHeading?.content).join(","));

  return <HeadingSection tag={tag} title={title} subTitle={sub_title} />;
};

export default AllCourseHeading;
